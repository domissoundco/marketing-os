...
// Posts (with attached images) and tasks live inside the brand record.
...
export async function saveBrand(brand) {
  ...
  await put(brandKey(brand.slug), JSON.stringify(record, null, 2), {
    ...
    allowOverwrite: true,
    cacheControlMaxAge: 0, // ← important: brand JSON changes often; do not CDN-cache.
  });
...
export async function deletePost(slug, postId) {
  ...
  // Best-effort delete any attached images from blob storage.
  if (post && post.images?.length) {
    await Promise.all(
      post.images.map((img) =>
        del(img.url).catch((err) =>
          console.error('Failed to delete image blob:', img.url, err.message)
        )
      )
    );
  }
