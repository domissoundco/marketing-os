...
// Best-effort schema migration in memory: normalises old records.
function migrate(brand) {
  if (!brand) return brand;
  brand.products = brand.products || [];
  brand.posts = (brand.posts || []).map((p) => {
    // Old status "published" → "posted"
...
export async function addProduct(slug, product) {
  ...
  const newProduct = {
    ...
    status: product.status || 'active', // active | paused | retired
    createdAt: now,
    updatedAt: now,
  };
...
export async function deleteProduct(slug, productId) {
  ...
  // Untag any posts pointing at this product.
  brand.posts = (brand.posts || []).map((p) => ({
    ...p,
...
// Latest stats entry for a post (or null).
